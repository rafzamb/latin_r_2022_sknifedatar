...
```{r, echo=FALSE, out.width='35%', out.height='35%', fig.align='center'}
...
Exponer las distintas funcionalidades de {skinifedatar} para el modelado y visualización de series de tiempo, mediante un caso de uso referente a la movilización vial en la ciudad de Buenos Aires para el periodo 2021-2022.
...
data_foods3 <- data %>% 
  ...
  select(-dept_id) %>% ungroup()
```


```{r}
data_foods3 %>% head(5) %>% kableExtra::kable(format = "html")
```
...
### Refit y pronostico (proximo mes)
...
```{r}
nest_data <- data %>% ungroup() %>% nest(nested_column = -dept_id)
```
...
```{r}
receta <- recipe(value ~ date, data = data %>% select(-dept_id )) %>%
...
# Workflow: prophet boosted
m_prophet_boost <- workflow() %>% add_recipe(receta) %>% 
...
### Refit y pronostico (próximos 6 meses)
...
```{r, message=FALSE, echo=FALSE, fig.width=10, fig.height=5, eval=FALSE}
...
```{r, echo=FALSE, fig.width=10, fig.height=5, eval=FALSE}
plots$plot[[2]]
```
]
...
```{r, echo=FALSE, fig.height=6, fig.width=14, fig.align='center', eval=FALSE}
wfs_multiforecast %>% 
  select(dept_id, nested_forecast) %>% 
  unnest(nested_forecast) %>% 
  group_by(dept_id) %>% 
